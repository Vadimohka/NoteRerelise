...
import com.example.note.helpers.NotesDatabaseHelper;
...
public class NotesDatabaseAdapter {
    ...
    private Cursor getAllEntries() {
        String[] columns = new String[] {NotesDatabaseHelper.COLUMN_ID,
                ...
                NotesDatabaseHelper.COLUMN_TAGS, NotesDatabaseHelper.COLUMN_DATE1,
                NotesDatabaseHelper.COLUMN_DATE2};
    ...
    public List<Note> getNotes() {
        ...
        if(cursor.moveToFirst()) {
            do {
                ...
                String date1 = cursor.getString(cursor.getColumnIndex(NotesDatabaseHelper.COLUMN_DATE1));
                notes.add(new Note(id, title, body, tags, date1));
            }
    ...
    private ContentValues getContentValues(Note note) {
        ...
        cv.put(NotesDatabaseHelper.COLUMN_DATE1,
                new SimpleDateFormat("dd-MM-yyyy HH:mm:ss").format(new Date()));
        cv.put(NotesDatabaseHelper.COLUMN_DATE2,
                new SimpleDateFormat("dd-MM-yyyy HH:mm:ss").format(new Date()));
        return cv;
    }
}